...
(() => {
  ...
  const stylesheet =
    ...
    // A host page that owns its own image controls opts out with no-controls.
    ':host([no-controls]) .ctl{display:none}' +
    ':host([data-filled][data-editable]:hover:not([no-controls])) .ctl,:host([data-reframe]:not([no-controls])) .ctl' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    // Public API for host pages that want their own always-visible controls
    // instead of the hover overlay: clear() empties the slot (and its stored
    // value), replace() opens the file picker, edit() enters reframe.
    clear() {
      this._gen++;
      this._swapGen = 0;
      ...
      else { subs.forEach((fn) => fn()); }
      this._render();
    }

    replace() { this._input.click(); }

    edit() { if (this.hasAttribute('data-filled')) this._enterReframe(); }
